...
extern unsigned char starcolr[1024], starcolg[1024], starcolb[1024];

unsigned char rcol[nblob],gcol[nblob],bcol[nblob];
...
// ==== Flower Precalcs ====
	...
	for (i=0; i<NSHAPES; i++)
	{
		...
		shape[i].rdiv=rand()%2 + 1;
		shape[i].gdiv=rand()%2 + 1;
		shape[i].bdiv=rand()%2 + 1;
